...
function buildExam() {
  ...
  for (const moduleKey of moduleOrder) {
    ...
    if (pool.length < needed) {
      throw new Error(
        `No hay suficientes preguntas en ${moduleTitles[moduleKey]}. Hay: ${pool.length}, hacen falta: ${needed}`
      );
    }
        ...
        ${moduleOrder.map((moduleKey) => {
          ...
          return `
            <div class="module-mini-card">
              ...
              <div class="module-mini-pass">mínimo ${rule.pass}</div>
            </div>
          `;
        }).join('')}
      </div>
...
function renderExamView() {
  ...
  app.innerHTML = `
    <div class="page">
      <header class="topbar">
        <div>
          <h1>Simulador Examen VTC</h1>
          <p>Pregunta ${state.currentIndex + 1} / ${state.examQuestions.length}</p>
        </div>

        <div class="topbar-actions">
          <button id="new-exam-btn" class="secondary-btn">Nuevo examen</button>
        </div>
      </header>
    ...
    .map(
      (m) => `
        <div class="summary-card ${m.passed ? 'pass' : 'fail'}">
          ...
          <p>Mínimo: ${m.pass}</p>
          <strong>${m.passed ? 'Apto' : 'No apto'}</strong>
        </div>
      `
    )
    .join('');
        ...
        <section class="module-section">
          <div class="module-header">
            ...
            <p>
              ...
              • mínimo ${moduleSummary.pass}
              • <strong>${moduleSummary.passed ? 'Apto' : 'No apto'}</strong>
            </p>
          </div>
      ...
      <header class="topbar">
        <div>
          ...
          <p class="final-status ${overallPassed ? 'pass' : 'fail'}">
            ${overallPassed ? 'Resultado final: Apto' : 'Resultado final: No apto'}
          </p>
        </div>
        <div class="topbar-actions">
